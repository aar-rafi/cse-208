...
class MinBinomialHeap<T> {
  ...
  *****************************************************************************/
  ...
  private removeSmallestRoot(): BinomialNode<T> | null {
    ...
    let min = cur;
    let prevMin: BinomialNode<T> | null = null;
    cur = cur.sibling;
    ...
    while (cur) {
      ...
      if (currentIsLessThanMin) {
        min = cur;
        prevMin = prev;
        //Type 'BinomialNode<T>' is not assignable to type 'null'.ts(2322)
      }
  ...
  *****************************************************************************/
  ...
  union(otherHeap: MinBinomialHeap<T>): MinBinomialHeap<T> {
    ...
    while (nextRoot !== null) {
      ...
      const nextTwoRootsAreEqual =
        nextRoot.sibling !== null &&
        nextRoot.sibling.degree === nextRoot.degree;
  ...
  private mergeForests(
    heapA: MinBinomialHeap<T>,
    heapB: MinBinomialHeap<T>
  ): MinBinomialHeap<T> {
    if (!heapA.head) return heapB;
    if (!heapB.head) return heapA;

    let head: BinomialNode<T> | null = null;
